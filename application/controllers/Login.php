		...
		if($this->form_validation->run() != true){
			...
			if($data->num_rows() > 0){
				...
				$where = array(
					...
					$cekdata2 = mysqli_query($db, "SELECT * FROM tbl_mahasiswa WHERE mhs_nim = '$uname' AND mhs_status_verifikasi = 'oke'");
					$data_mhs = $this->db->query("SELECT * FROM tbl_mahasiswa WHERE mhs_nim = '$uname' AND mhs_nosk_yudisium != ''")->num_rows();
					if ($data->num_rows() > 0) {
					$mydata = $data->row();
					$session = array(
					...
					$this->session->set_userdata($session);
						if($mydata->peserta_status_verifikasi == 'oke'){
							if ($data_mhs > 0) {
								redirect('wisuda');
							}else{
								$this->session->set_flashdata('msg','Tidak dapat login, anda belum memiliki Nomor SK Yudisium');
						 ...
						 $this->cek_login($uname,$aktif,$s1,$data_mhs,$cekdata2);
					}
				}else{
					$where = array(
							'mhs_nim' => $uname,
