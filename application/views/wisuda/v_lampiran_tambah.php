      ...
      <div class="tile">
        ...
        <div class="tile-body">
          ...
          foreach($edit As $a){
              ...
              $res1=mysqli_query($db, "SELECT * FROM tbl_lampiran WHERE lampiran_keperluan = 'Wisuda' AND lampiran_prodi LIKE '%".$a->peserta_prodi."%' ORDER BY lampiran_id ASC");
              $res3=mysqli_query($db, "SELECT b.peserta_id FROM tbl_lampiran a, tbl_peserta_lampiran b WHERE a.lampiran_id = b.peserta_lamp_kode AND a.lampiran_keperluan = 'Wisuda' ");
          ...
          ?>
          <form method="post" action="<?php echo base_url('wisuda/lampiran_add_act') ?>" enctype="multipart/form-data">
            <table width="100%" border="0">
              <tr>
                <td width="50%" align="left" valign="top">
                  <?php
                  $berkas_lamp = mysqli_num_rows($res1);
                  $berkas_upload = mysqli_num_rows($res3) + 1;
                  echo '<input type="hidden" name="berkas_lamp" value="'.$berkas_lamp.'">';
                  echo '<input type="hidden" name="berkas_upload" value="'.$berkas_upload.'">';
          ...
          <?php } ?>
            <div class="tile-footer">
              <a href="<?php echo base_url('wisuda/biodata') ?>" class="btn btn-primary"><i class="fa fa-arrow-left"></i> Kembali</a>
            </div>
        </div>
      </div>
    </div>
