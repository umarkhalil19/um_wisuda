<!-- Essential javascripts for application to work-->
      ...
      <script type="text/javascript">
			$(document).ready(function(){
				$('#provinsi').change(function(){
					var id=$(this).val();
					$.ajax({
						url : "<?php echo base_url();?>mahasiswa/kabupaten",
						method : "POST",
						data : {id: id},
						async : false,
				        dataType : 'json',
						success: function(data){
				            ...
				            for(i=0; i<data.length; i++){
				                html += '<option value="'+data[i].kota_kabupaten+'">'+data[i].kota_kabupaten+'</option>';
				            }
				            $('#kabupaten').html(html);
		...
		<script type="text/javascript">
			$(document).ready(function(){
				$('#kabupaten').change(function(){
					var id=$(this).val();
					$.ajax({
						url : "<?php echo base_url();?>mahasiswa/kecamatan",
						method : "POST",
						data : {id: id},
						async : false,
				        dataType : 'json',
						success: function(data){
				            ...
				            var i;
				            for(i=0; i<data.length; i++){
				                html += '<option value="'+data[i].kota_kecamatan+'">'+data[i].kota_kecamatan+'</option>';
				            }
				            $('.kecamatan').html(html);
	...
	<script type='text/javascript'>
        ...
        function namamhs(kode)
        {
            var kode = kode.value;
            if (!kode) return;
            xmlhttp.open('get', '<?php echo base_url('index/panggil_nama_mhs?kode=') ?>'+kode, true);
            xmlhttp.onreadystatechange = function() {
                if ((xmlhttp.readyState == 4) && (xmlhttp.status == 200))
                {
